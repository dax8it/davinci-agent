...
description: Clean and level spoken introductions, stage banter, or interviews in DaVinci Resolve 19 while preserving editable processing and sync. Use for dialogue only; live band music and audience ambience need a performance mix.
---
...
On live band and venue projects, identify an isolated spoken segment or dialogue track first. Do not apply a speech gate, Voice Isolation, speech loudness target, or silence removal across music, singing, applause, crowd reactions, or reverberation tails. Never ripple-delete time as part of cleanup. Preserve the continuous performance master and its sync to picture.
...
4. Use approximately -16 LUFS and below -1.5 dBTP only as provisional voice-only working targets. The final mixed performance target depends on the brief. Measure the actual encoded output instead of predicting it from fader values.
5. Keep effects live by default. If the user requests baked audio, or measured playback problems require it, render a separate full-coverage 48 kHz, 24-bit PCM WAV with a versioned filename. Preserve channel layout and duration, measure content alignment/latency, retain the original and editable chain, and bypass processing already baked into the derivative. Do not force a bounce of every imported performance clip.
